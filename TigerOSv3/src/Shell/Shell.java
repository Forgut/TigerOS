	...
	void COMMANDEXEC() throws IOException
	{
		...
		switch(User_Command)
		{
			case("help"):
			{
				...
				System.out.println("lsf   display main catalog");
				
				
				System.out.println("cf x   create file x");
		...
		switch(User_Command.substring(0, 4))
		{
			case("disk"):
			{
				...
				case('r'):
				{
					...
					filesystem.showRootEntry(UserInput.readLine().trim());
					break;
				}
				case('s'):
				{
					System.out.println("Enter the number of the block");
					int x = Integer.parseInt(UserInput.readLine().trim());
